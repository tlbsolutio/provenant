#!/usr/bin/env node
// Provenant — publish a rendered report to an accessible HTTPS page.
...
// Usage (lib): import { publish } from "./publish.mjs"
//
...
export function publish({ htmlPath, html, id, dir } = {}) {
  ...
  if (wantVercel && hasVercelCli()) {
    ...
    if (base) return { url: `${base}/${id}.html`, indexUrl: `${base}/`, dir: root, deployed: true };
    return { url: null, dir: root, page, deployed: false, reason: "vercel deploy returned no URL", log: out.slice(-400) };
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const r = publish({ htmlPath: f, id: get("--id"), dir: get("--dir") });
  if (r.url) console.log(`published → ${r.url}`);
  else console.log(`not deployed (${r.reason})${r.page ? `\nlocal: ${r.page}` : ""}`);
}
